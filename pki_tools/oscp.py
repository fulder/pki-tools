...
from . import cert_from_pem, ExtensionMissing, Error, Revoked


class OcspFetchFailure(Error):
    pass

def check_revoked(cert_pem: str, issuer_cert: str):
    cert = cert_from_pem(cert_pem)
    issuer_cert = cert_from_pem(cert_pem)
    check_revoked_crypto_cert(cert, issuer_cert)


def check_revoked_crypto_cert(cert: x509.Certificate, issuer_cert: x509.Certificate):
    builder = ocsp.OCSPRequestBuilder()
    builder = builder.add_certificate(cert, issuer_cert, SHA256())
    req = builder.build()
    req_path = base64.b64encode(req.public_bytes(serialization.Encoding.DER)).decode("ascii")
...
def _get_ocsp_status(uri) -> OCSPResponse:
    ...
    if ret.status_code != 200:
        raise OcspFetchFailure(f"Unexpected response status code: {ret.status_code}")

    ocsp_res = ocsp.load_der_ocsp_response(ret.content)
    if ocsp_res.response_status != OCSPResponseStatus.SUCCESSFUL:
        raise OcspFetchFailure(f"Invalid OCSP Response status: {ocsp_res.response_status}")
